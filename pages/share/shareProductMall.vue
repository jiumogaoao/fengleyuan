...
<script>
	...
	export default {
		...
		onReady(){
			let _this=this;
			this.url='/pages/product/product?id='+this.$api.prePage().id
			this.productImg=this.$api.prePage().thumb_url[0]
			this.oldPrice=this.$api.prePage().reserve_price
			this.title=this.$api.prePage().title
			this.price=this.$api.prePage().showSKU.zk_final_price
			this.couponprice=this.$api.prePage().coupon_face_value||0
			uni.showLoading({
				title:'正在生产分享图'
			})
