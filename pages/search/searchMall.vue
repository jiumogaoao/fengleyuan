			...
			<view class="goods-list" v-if="goodsList.length">
				...
				>
					...
					<view class="price-box">
						...
						<text class="right">已售 {{item.salescout}}</text>
					</view>
				</view>
				<view
					v-for="(item, index) in goodsList" :key="index"
	...
	export default {
		...
		methods:{
			...
			search(){
				...
				this.curr_page = 1;
				postFetch('index.php/index/indexgoods/index',{keywork:this.keywork,curr_page:this.curr_page,type:this.salse,classid:this.classid},false,function(res){
					console.log("search",res)
					
					if(res.data.list){
						_this.$set(_this,'goodsList',res.data.list)
						_this.curr_page += 1
					}
				})
			},
			searchPush(){
				let _this=this;
				postFetch('index.php/index/indexgoods/index',{keywork:this.keywork,curr_page:this.curr_page,type:this.salse},false,function(res){
					console.log("search",res)
